...
class StreamingTopologyTest : TestCase() {
  ...
  fun `test total`() {
    ...
    TopologyTestDriver(builder.build(), StreamingTopology.config()).use { testDriver ->
      testDriver.pipeInput(
          createTransactionRecord(Transaction("dean", 150, "", Date().time)))
      testDriver.pipeInput(
          createTransactionRecord(Transaction("alice", 6, "", Date().time)))
      testDriver.pipeInput(
          createTransactionRecord(Transaction("dean", 100, "", Date().time)))
  ...
  fun `test rolling total in same window`() {
    ...
    TopologyTestDriver(builder.build(), StreamingTopology.config()).use { testDriver ->
      ...
      testDriver.pipeInput(createTransactionRecord(Transaction("dean", 150, "", now)))
      testDriver.pipeInput(createTransactionRecord(Transaction("dean", 100, "", now)))
  ...
  fun `test rolling total in different window`() {
    ...
    TopologyTestDriver(builder.build(), StreamingTopology.config()).use { testDriver ->
      ...
      testDriver.pipeInput(createTransactionRecord(Transaction("dean", 150, "", now)))
      testDriver.pipeInput(
          createTransactionRecord(Transaction("dean", 100, "", now + 1000 * 40)))
